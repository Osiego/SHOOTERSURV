    ...
    def connect_rooms(rooms):
        ...
        for i in range(len(rooms)):
            for j in range(i + 1, len(rooms)):
                (x1, y1, w1, h1) = rooms[i]
                (x2, y2, w2, h2) = rooms[j]
                if x1 < x2 + w2 and x1 + w1 > x2 and y1 < y2 + h2 and y1 + h1 > y2:  # Check for overlap
                    continue  # Skip if rooms overlap
                if random.randint(0, 1):
                    connections.append((x1 + w1 // 2, y1 + h1 // 2, x2 + w2 // 2, y2 + h2 // 2))
                else:
                    connections.append((x2 + w2 // 2, y2 + h2 // 2, x1 + w1 // 2, y1 + h1 // 2))
        return connections
